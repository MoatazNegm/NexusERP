...
export const TechnicalReviewModule: React.FC<TechnicalReviewModuleProps> = ({ config, refreshKey, currentUser }) => {
  ...
  const handleToggleAcceptance = async (item: CustomerOrderItem) => {
    ...
    const updated = await dataService.toggleItemAcceptance(selectedOrder.id, item.id, currentUser.username);
    setSelectedOrder(updated);
  ...
  const handleFinalizeReview = async () => {
    ...
    setIsProcessing(true);
    try {
      await dataService.finalizeTechnicalReview(selectedOrder.id, currentUser.username);
  ...
  const handleRollback = async () => {
    ...
    setIsProcessing(true);
    try {
        await dataService.rollbackOrderToLogged(selectedOrder.id, rollbackReason, currentUser.username);
        setSelectedOrder(null);
        setSelectedItem(null);
                                    ...
                                    <tr key={c.id} className="group hover:bg-slate-50/50 transition-colors">
                                       ...
                                       {!selectedItem.isAccepted && (
                                         <td className="px-6 py-4 text-right">
                                            <button 
                                              onClick={() => dataService.removeComponent(selectedOrder.id, selectedItem.id, c.id, config.settings.minimumMarginPct, currentUser.username).then(o => { setSelectedOrder(o); setSelectedItem(o.items.find(it => it.id === selectedItem.id)!); })}
