  ...
  const handleExecuteAction = async () => {
    ...
    if (type === 'ORDER_ROLLBACK') {
      ...
      try {
        ...
        await dataService.rollbackOrderToLogged(order.id, resetReason, currentUser.username);
        await fetchData();
        closeModal();
    ...
    try {
      ...
      await dataService.updateComponent(order.id, item.id, comp.id, updates, config.settings.minimumMarginPct, currentUser.username);
      await fetchData();
      closeModal();
    } catch (e: any) {
            ...
            <div key={c.id} className="flex flex-col lg:flex-row justify-between items-center p-6 bg-slate-50 rounded-[2rem] border border-slate-100 hover:border-blue-200 transition-all group">
               <div className="flex gap-6 items-center w-full lg:w-auto">
                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-xl shadow-inner ${
                    c.status === 'ORDERED' ? 'bg-emerald-50 text-emerald-600' : 
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                       ...
                       <span className={`px-2 py-0.5 text-[8px] font-black rounded uppercase ${
                         c.status === 'ORDERED' ? 'bg-emerald-600 text-white' : 
           ...
           <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-xl p-10 my-8 animate-in zoom-in-95 duration-300 border border-slate-100">
              <div className="flex items-center gap-6 mb-8">
                 <div className={`w-16 h-16 rounded-3xl flex items-center justify-center text-3xl shadow-inner ${
                   activeAction.type === 'RFP' ? 'bg-blue-50 text-blue-600' :
                   ...
                   activeAction.type === 'RESET' || activeAction.type === 'ORDER_ROLLBACK' ? 'bg-rose-50 text-rose-600' : 'bg-emerald-50 text-emerald-600'
                    ...
                    <i className={`fa-solid ${
                      activeAction.type === 'RFP' ? 'fa-paper-plane' :
              ...
              <div className="mt-10 flex gap-3">
                 ...
                 <button 
                   ...
                   className={`flex-[2] py-4 rounded-2xl font-black text-[10px] uppercase shadow-xl transition-all flex items-center justify-center gap-2 ${
                     activeAction.type === 'RESET' || activeAction.type === 'ORDER_ROLLBACK' ? 'bg-rose-600 hover:bg-rose-700 text-white' : 'bg-blue-600 hover:bg-blue-700 text-white shadow-blue-100'
                   }`}
                 >
