...
### 7. Procurement PO Reference Display, Project/Non-Project Tagging & Universal Search
- **PO Reference Header Badge:** In `ProcurementModule.tsx`, every order card header prominently displays the customer's PO reference badge (`PO: <customerReferenceNumber>`) alongside the internal order number in both the **Trade/Manufacturing** (`purchases`) and **Outsourcing** (`outsourcing`) views.
- **Project Name vs Non-Project Indicator:** Order card headers explicitly display the project name (`Project: <projectName>`) with a distinct violet badge and icon when present, or a clear `Non-Project` indicator when no project is attached.
- **Universal Multi-Field Search Box:** A dedicated search input in the Procurement and Order Management section headers filters orders in real time across all visible attributes, including internal order numbers, customer PO references, customer names, project names (and explicit `non-project` / `project` keywords), item order numbers & descriptions, part numbers, component descriptions, supplier names, supplier part numbers, PO numbers, RFP IDs, and manufacturing steps.
